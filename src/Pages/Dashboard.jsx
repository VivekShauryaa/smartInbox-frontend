...
// Section Component
// eslint-disable-next-line react/prop-types
const Section = ({ title, children, onRefresh }) => (
  <section className="bg-white p-6 rounded-2xl shadow-md">
    <div className="flex justify-between items-center mb-4">
      <h2 className="text-xl font-semibold text-gray-800">{title}</h2>
      {title === "Quick Bits" && onRefresh && (
        <button className="text-xl" onClick={onRefresh}>
          <FcRefresh />
        </button>
      )}
    </div>
...
const Dashboard = () => {
  ...
  const fetchDashboardMetrics = async () => {
    try {
      ...
      const pipelineData = [
        ...
        {
          stage: "Responses Generated",
          value: raw.totalResponsesGenerated || 0,
        },
  ...
  const fetchSummary = async () => {
    try {
      const res = await contextRequest.get(`/fetchSummary/${email}`);
      console.log(res.data);
      setSummaryList(res.data.summaries || []);
    } catch (err) {
  ...
  return (
    <div className="min-h-screen p-2 px-6 space-y-10">
        ...
        <div className="space-y-6">
          <Section title="User Context">
            <div className="space-y-2 text-gray-700 text-sm">
              <p>
                <span className="font-medium">Name:</span> {userContext.name}
              </p>
              <p>
              ...
              <p>
                ...
                <span
                  className={`font-medium ${
                    tokenExists ? "text-green-600" : "text-red-500"
                  }`}
                >
      ...
      <Section title="Quick Bits" onRefresh={UpdateSummary}>
  ...
  <div className="space-y-4">
    {smartReplies.length === 0 ? (
      ...
      smartReplies.slice(-5).map((reply, idx) => {
        try {
          const [fromLine, messageLine, responseLineRaw] = reply.split(" | ");
          const responseJson = responseLineRaw?.includes("Response: ")
            ? responseLineRaw.split("Response: ")[1]
            : null;
          const parsedResponse = responseJson ? JSON.parse(responseJson) : {};
          ...
          return (
            <div key={idx}>
              <div className="bg-white p-4 rounded-xl shadow bg-gradient-to-r from-white to-blue-50 border border-blue-100">
                ...
                {expandedIndex === idx && (
                  <div className="mt-4 bg-gray-50 p-4 rounded-lg shadow-md space-y-2">
          ...
          console.error("Error parsing smart reply:", err);
          return (
            <div key={idx} className="bg-red-100 text-red-600 p-4 rounded shadow">
              <p>Error rendering this smart reply. Check data format.</p>
            </div>
          );
        }
      })
